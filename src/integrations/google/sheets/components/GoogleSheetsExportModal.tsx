        ...
        {!exportResult ? (
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-ink-navy">Tùy chọn xuất dữ liệu:</label>
            ...
            {exportMode === 'existing_sheet' && (
              <div className="space-y-1.5 p-3 bg-cloud border border-hairline rounded-lg">
                <label className="text-xs font-medium text-ink-navy">Chọn Bảng tính đích:</label>
                {connectedSheets.length === 0 ? (
                  ...
                  >
                    <SelectTrigger className="w-full h-9 text-xs bg-white border-hairline text-ink-navy">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-semibold text-ink-navy flex items-center gap-1.5">
                  <Layers className="w-3.5 h-3.5 text-mist-gray" /> Các cột sẽ được xuất ({fields.length} cột):
                </label>
                {module === 'finance' && (
              ...
              <div className="flex flex-wrap gap-1.5 p-2.5 bg-cloud border border-hairline rounded-lg max-h-36 overflow-y-auto">
                {fields.map((f) => (
                  <span
                    key={f.key}
                    className="inline-flex items-center text-[11px] px-2 py-1 bg-white border border-hairline rounded text-slate-gray font-medium shadow-xs"
                  >
                    {f.label}
            ...
            {/* Notice */}
            <div className="flex items-start gap-2 p-2.5 bg-[#e6f0ff] border border-[#d4e4fa] rounded-lg text-ink-navy text-xs leading-relaxed">
          ...
          <div className="py-4 space-y-4 text-center">
            ...
            <div>
              <h3 className="text-base font-semibold text-ink-navy">Xuất dữ liệu thành công!</h3>
              <p className="text-xs text-slate-gray mt-1">
                Đã xử lý <strong>{exportResult.rowCount}</strong> bản ghi từ mô-đun {moduleInfo?.title}
              </p>
            </div>

            <div className="p-3 bg-cloud border border-hairline rounded-lg text-left text-xs space-y-2">
              <div className="flex items-center justify-between text-slate-gray">
                <span>Tên tab:</span>
                <span className="font-semibold text-ink-navy">{exportResult.sheetName}</span>
              </div>
              <div className="flex items-center justify-between text-slate-gray">
                <span>Thời gian:</span>
                <span className="font-mono text-ink-navy">{new Date(exportResult.exportedAt).toLocaleTimeString('vi-VN')}</span>
              </div>
            </div>
