...
export function GoogleSheetsManagerCard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-hairline rounded-2xl shadow-sm bg-white">
        <CardHeader className="pb-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="h-11 w-11 rounded-xl bg-signal-blue text-white flex items-center justify-center shadow-xs shrink-0">
                <FileSpreadsheet strokeWidth={1.5} className="w-6 h-6" />
              </div>
              <div>
                <CardTitle className="text-base font-semibold text-ink-navy flex items-center gap-2.5 flex-wrap">
            ...
            {canManage && (
              <Button
                ...
                size="sm"
                className="bg-signal-blue hover:bg-[#005be0] text-white text-xs gap-1.5 self-start sm:self-auto rounded-xl shadow-xs"
              >
        ...
        <CardContent className="space-y-4">
          ...
          <div className="p-3.5 bg-cloud border border-hairline rounded-xl text-xs text-slate-gray space-y-1.5">
          ...
          <div>
            <div className="text-xs font-semibold text-ink-navy mb-2 flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {[
                ...
                return (
                  <div
                    key={item.module}
                    className="p-3.5 bg-white rounded-xl border border-hairline hover:border-[#d4e4fa] transition-all space-y-2.5 shadow-xs"
                  >
                    <div className="flex items-center gap-2 font-medium text-xs text-ink-navy">
                      <div className="p-1.5 rounded-lg bg-[#e6f0ff] text-signal-blue border border-[#d4e4fa]">
                        <IconComponent className="w-4 h-4" />
                      </div>
                      <div>
                        <div className="font-semibold">{item.title}</div>
                        <div className="text-[11px] text-slate-gray">{item.desc}</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 pt-1 border-t border-hairline">
                      <Button
                        ...
                        className="flex-1 h-7.5 text-[11px] px-2 gap-1 rounded-lg border-hairline text-slate-gray hover:text-signal-blue hover:bg-[#e6f0ff] shadow-xs"
                      >
                        <Download className="w-3 h-3 text-signal-blue" /> Xuất Sheet
                      </Button>
                      {canManage && (
                        <Button
                          variant="outline"
                          ...
                          className="flex-1 h-7.5 text-[11px] px-2 gap-1 rounded-lg border-hairline text-slate-gray hover:text-signal-blue hover:bg-[#e6f0ff] shadow-xs"
                        >
                          <Upload className="w-3 h-3 text-signal-blue" /> Nhập Sheet
                        </Button>
                      )}
                    </div>
                  </div>
          ...
          <div className="space-y-2 pt-3 border-t border-hairline">
            <div className="flex items-center justify-between">
              <div className="text-xs font-semibold text-ink-navy">
                Bảng tính Google Sheets đã liên kết ({spreadsheets.length}):
              </div>
            </div>

            {isLoading ? (
              <div className="py-6 text-center text-xs text-mist-gray">Đang tải danh sách bảng tính...</div>
            ) : spreadsheets.length === 0 ? (
              <div className="p-6 text-center border border-dashed border-hairline rounded-lg text-mist-gray text-xs space-y-2">
                <FileSpreadsheet strokeWidth={1.5} className="w-8 h-8 text-mist-gray mx-auto" />
              ...
              <div className="border border-hairline rounded-xl divide-y divide-hairline overflow-hidden bg-white shadow-xs">
                {spreadsheets.map((sheet) => (
                  <div
                    key={sheet.id}
                    className="p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 hover:bg-cloud transition-colors"
                  >
                    <div className="flex items-start gap-2.5 min-w-0">
                      <div className="p-2 rounded-lg bg-[#e6f0ff] text-signal-blue border border-[#d4e4fa] shrink-0 mt-0.5">
                        <FileSpreadsheet className="w-4 h-4" />
                      </div>
                      <div className="min-w-0">
                        <div className="font-semibold text-xs text-ink-navy flex items-center gap-1.5">
                          <span className="truncate">{sheet.spreadsheetName}</span>
                          <span className="px-1.5 py-0.5 rounded-md text-[10px] font-medium bg-pebble text-slate-gray">
                            {sheet.status === 'active' ? 'Đang kết nối' : 'Đã lưu trữ'}
                          </span>
                        </div>
                        <div className="text-[11px] text-mist-gray font-mono truncate max-w-sm">
                          ID: {sheet.spreadsheetId}
                        </div>
                        <div className="flex items-center gap-3 text-[11px] text-mist-gray mt-1">
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-center">
                      <Button
                        ...
                        className="h-8 text-xs px-2.5 gap-1.5 rounded-lg border-hairline text-slate-gray hover:text-signal-blue hover:bg-[#e6f0ff] shadow-xs"
                      >
                        <ExternalLink className="w-3.5 h-3.5 text-mist-gray" /> Mở Sheet
                      </Button>
      ...
      <Dialog open={linkDialogOpen} onOpenChange={setLinkDialogOpen}>
        <DialogContent className="sm:max-w-lg rounded-3xl p-5 pr-10 sm:p-6 sm:pr-12 border border-hairline shadow-xl">
          <form onSubmit={handleLinkSubmit}>
            ...
            <div className="space-y-3.5 py-3.5">
              <div className="space-y-1">
                <label className="text-xs font-semibold text-ink-navy">Tên gọi bảng tính <span className="text-rose-500">*</span></label>
                <Input
                  required
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-ink-navy">Đường dẫn Google Sheet (URL hoặc ID) <span className="text-rose-500">*</span></label>
                <Input
                  required
