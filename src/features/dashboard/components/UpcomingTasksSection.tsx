          ...
          <div className="divide-y divide-hairline">
            {tasks.map((task) => {
              const priorityMeta = TASK_PRIORITY_META[task.priority] || {
                ...
                bgClass: 'bg-pebble border-hairline text-slate-gray',
              };

              const statusMeta = TASK_STATUS_META[task.status] || {
                label: task.status,
                colorClass: 'text-ink-navy',
                bgClass: 'bg-pebble text-ink-navy',
              };
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="space-y-1 min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <Badge
                          variant="outline"
                          className={`text-[11px] py-0 px-2 rounded-full font-semibold border ${priorityMeta.bgClass}`}
                        >
                          {priorityMeta.label}
                        </Badge>
                        ...
                        {task.activity && (
                          <span className="text-xs text-signal-blue font-medium truncate max-w-[180px]">
                      ...
                      <div className="flex items-center gap-3 text-xs text-slate-gray flex-wrap pt-0.5">
                        {task.dueDate ? (
                          <div className={`flex items-center gap-1.5 ${task.isOverdue ? 'text-rose-600 font-semibold' : ''}`}>
                            ...
                            <span className="tabular-nums">Hạn: {formatShortDate(task.dueDate)}</span>
                            {task.isOverdue && (
                          ...
                          <span className="text-mist-gray">Không có hạn</span>
                        )}

                        <div className="flex items-center gap-1.5 text-ink-navy font-medium">
                          {task.assignee?.avatarUrl ? (
                            ...
                            <div className="w-4 h-4 rounded-full bg-pebble text-[9px] font-bold text-ink-navy flex items-center justify-center border border-hairline">
                        ...
                        {task.progress !== undefined && (
                          <div className="flex items-center gap-2 text-xs text-slate-gray">
                            <div className="w-14 bg-pebble border border-hairline rounded-full h-1.5 overflow-hidden">
                              <div
                                className="bg-signal-blue h-1.5 rounded-full"
                                style={{ width: `${Math.min(100, Math.max(0, task.progress))}%` }}
                              />
                            </div>
                            <span className="font-semibold text-ink-navy tabular-nums text-[11px]">{task.progress}%</span>
                          </div>
                        )}
                      </div>
                    </div>
