...
export function FinanceTransactionCard({
  ...
  return (
    <div
      ...
      className={`bg-white rounded-2xl border p-4 shadow-xs hover:border-[#d4e4fa] hover:shadow-xs transition-all space-y-2.5 cursor-pointer flex flex-col justify-between ${
        isPending ? 'border-amber-300 bg-amber-50/20' : 'border-hairline'
      }`}
    >
      <div className="space-y-2">
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {isLocked && (
              <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-slate-gray bg-cloud px-1.5 py-0.5 rounded-full border border-hairline">
        ...
        {isRejected && tx.rejectionReason && (
          <div className="p-2 bg-rose-50 border border-rose-200 rounded-xl text-[11px] text-rose-800 flex items-start gap-1.5">
      ...
      <div className="pt-2 border-t border-hairline flex items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-1.5 text-slate-gray text-[11px]">
          <Calendar className="h-3 w-3 text-mist-gray" />
          <span className="tabular-nums">{formatDate(tx.transactionDate)}</span>
        </div>
