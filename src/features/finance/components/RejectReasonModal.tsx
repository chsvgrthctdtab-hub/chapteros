...
export function RejectReasonModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink-navy/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div
        className="w-full max-w-lg bg-white rounded-2xl shadow-xl border border-hairline overflow-hidden animate-in zoom-in-95 duration-200"
        role="dialog"
        aria-modal="true"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-hairline bg-rose-50/50">
          ...
          <button
            ...
            onClick={onClose}
            disabled={isLoading}
            className="p-1 text-mist-gray hover:text-slate-gray rounded-lg hover:bg-cloud transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="p-3.5 bg-cloud border border-hairline rounded-xl space-y-1.5 text-xs text-slate-gray">
            <div className="flex justify-between items-center">
              <span className="text-mist-gray">Giao dịch:</span>
              <span className="font-semibold text-ink-navy line-clamp-1 max-w-[240px]">
                {transaction.description}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-mist-gray">Số tiền:</span>
              <span className="font-bold text-rose-600">
                {formatVND(transaction.amount)}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-mist-gray">Người tạo phiếu:</span>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3.5 py-2.5 text-sm bg-cloud border border-hairline rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 text-ink-navy transition-all placeholder:text-mist-gray"
            />
            {error && (
