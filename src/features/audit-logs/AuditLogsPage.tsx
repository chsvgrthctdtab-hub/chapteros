...
export function AuditLogsPage() {
  ...
  if (!isAuthorized) {
    return (
      <div id="audit-logs-page" className="space-y-6">
        ...
        <Card className="border-rose-200 bg-rose-50/30 max-w-2xl mx-auto my-12 shadow-xs">
          <CardContent className="p-8 text-center space-y-4">
            ...
            <div className="pt-2">
              <span className="text-xs text-mist-gray font-medium bg-white px-3 py-1.5 rounded-lg border border-hairline inline-flex items-center gap-1.5 shadow-xs">
                <Lock className="w-3.5 h-3.5 text-mist-gray" />
                <span>Vai trò hiện tại của bạn: <strong className="text-ink-navy">{role || 'Thành viên'}</strong></span>
              </span>
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <div id="audit-logs-page" className="space-y-5">
        ...
        actions={
          <div className="flex items-center gap-1.5 sm:gap-2">
            <Button
              ...
              className="text-xs h-8 px-2 sm:px-2.5 bg-white hover:bg-cloud border-hairline text-slate-gray hover:text-ink-navy shadow-xs font-semibold"
            >
              <Download className="w-3.5 h-3.5 sm:mr-1.5 text-mist-gray shrink-0" />
            ...
            <Button
              ...
              className="text-xs h-8 px-2 sm:px-2.5 bg-white hover:bg-cloud border-hairline text-slate-gray hover:text-ink-navy shadow-xs font-semibold"
            >
              <RotateCcw className={`w-3.5 h-3.5 sm:mr-1.5 shrink-0 ${isFetching ? 'animate-spin text-signal-blue' : ''}`} />
      ...
      {isLoading ? (
        <div className="bg-white rounded-xl border border-hairline p-6 space-y-4 shadow-xs">
          <div className="h-8 bg-cloud rounded-lg animate-pulse" />
          <div className="space-y-2">
            {[1, 2, 3, 4, 5, 6, 7].map((i) => (
              <div key={i} className="h-12 bg-cloud rounded-lg animate-pulse" />
            ))}
          </div>
        </div>
      ) : isError ? (
