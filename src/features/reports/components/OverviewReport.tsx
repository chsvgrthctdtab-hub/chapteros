    ...
    <div className="space-y-6" id="overview-report-section">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-ink-navy tracking-tight">
          ...
          <Card className="border-hairline shadow-xs bg-white">
            <CardHeader className="p-4 pb-3 border-b border-hairline flex flex-row items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <CardTitle className="text-xs font-bold text-ink-navy">Hoạt động & Sự kiện</CardTitle>
            ...
            <CardContent className="p-4 space-y-3">
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="rounded-lg bg-cloud p-2 border border-hairline">
                  <div className="text-base font-black text-emerald-600">
                    {activityStats ? activityStats.completedActivities : '---'}
                  </div>
                  <div className="text-2xs text-mist-gray mt-0.5 font-medium">Hoàn thành</div>
                </div>
                <div className="rounded-lg bg-cloud p-2 border border-hairline">
                  ...
                  <div className="text-2xs text-mist-gray mt-0.5 font-medium">Đang triển khai</div>
                </div>
                <div className="rounded-lg bg-cloud p-2 border border-hairline">
                  <div className="text-base font-black text-amber-600">
                    {activityStats ? activityStats.planningActivities + activityStats.draftActivities : '---'}
                  </div>
                  <div className="text-2xs text-mist-gray mt-0.5 font-medium">Kế hoạch / Dự thảo</div>
                </div>
              </div>

              {activityStats && activityStats.byCategory.length > 0 && (
                <div className="pt-2 border-t border-hairline">
                  <div className="text-2xs font-bold uppercase tracking-wider text-mist-gray mb-1.5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {activityStats.byCategory.slice(0, 4).map((c) => (
                      <span
                        key={c.category}
                        className="px-2 py-0.5 rounded-md text-2xs font-semibold bg-cloud text-slate-gray border border-hairline"
                      >
                        {c.category}: {c.count}
                      </span>
                    ))}
          ...
          <Card className="border-hairline shadow-xs bg-white">
            <CardHeader className="p-4 pb-3 border-b border-hairline flex flex-row items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <CardTitle className="text-xs font-bold text-ink-navy">Tiến độ Thực thi Nhiệm vụ</CardTitle>
                  <CardDescription className="text-2xs text-mist-gray">
            ...
            <CardContent className="p-4 space-y-3">
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-2xs">
                  <span className="font-semibold text-slate-gray">Tỷ lệ hoàn thành công việc</span>
                  <span className="font-bold text-emerald-700">
                    {taskStats ? `${taskStats.completionRate}%` : '---'}
                  </span>
                </div>
                <div className="w-full h-2 rounded-full bg-cloud overflow-hidden border border-hairline">
                  <div
              ...
              <div className="grid grid-cols-4 gap-1.5 pt-1 text-center">
                ...
                <div className="rounded-lg bg-[#e6f0ff] p-1.5 border border-[#d4e4fa]">
                  <div className="text-sm font-black text-signal-blue">
                    {taskStats ? taskStats.inProgressTasks : '---'}
                  </div>
                  <div className="text-3xs text-signal-blue font-medium">Đang làm</div>
                </div>
          ...
          <Card className="border-hairline shadow-xs bg-white">
            <CardHeader className="p-4 pb-3 border-b border-hairline flex flex-row items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <CardTitle className="text-xs font-bold text-ink-navy">Vị thế Ngân sách & Sổ quỹ</CardTitle>
                  <CardDescription className="text-2xs text-mist-gray">
            ...
            <CardContent className="p-4 space-y-3">
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-cloud border border-hairline">
                <span className="text-xs font-medium text-slate-gray">Số dư khả dụng</span>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="p-2 rounded-lg bg-emerald-50/50 border border-emerald-100 flex items-center justify-between">
                  <span className="text-2xs text-slate-gray flex items-center">
          ...
          <Card className="border-hairline shadow-xs bg-white">
            <CardHeader className="p-4 pb-3 border-b border-hairline flex flex-row items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <CardTitle className="text-xs font-bold text-ink-navy">Quy mô Hội viên & Nhân sự</CardTitle>
                  <CardDescription className="text-2xs text-mist-gray">
            ...
            <CardContent className="p-4 space-y-3">
              <div className="grid grid-cols-2 gap-2 text-center text-xs">
                ...
                <div className="rounded-lg bg-[#e6f0ff] p-2 border border-[#d4e4fa]">
                  <div className="text-base font-black text-signal-blue">
                    {memberStats ? memberStats.alumniMembers : Math.max(0, memberCount - activeMemberCount)}
                  </div>
                  <div className="text-2xs text-signal-blue font-medium">Cựu hội viên</div>
                </div>
              </div>

              {currentTerm && (
                <div className="p-2 rounded-lg bg-cloud border border-hairline flex items-center justify-between text-2xs">
                  <span className="text-mist-gray">Nhân sự trong nhiệm kỳ {currentTerm.name}:</span>
                  <span className="font-bold text-signal-blue">{currentTerm.memberCount} thành viên</span>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left border-collapse">
              ...
              <tbody className="divide-y divide-hairline">
                <tr className="hover:bg-cloud/60 transition-colors">
                  <td className="py-3 px-4 font-semibold text-ink-navy flex items-center gap-2">
                    <Users className="w-3.5 h-3.5 text-signal-blue" />
                    Nhân sự & Hội viên
                  </td>
                  <td className="py-3 px-3 font-bold text-slate-gray">
                    {memberCount} hội viên
                  </td>
                  <td className="py-3 px-3 text-slate-gray text-2xs">
                  ...
                  <td className="py-3 px-4 text-right">
                    <Link to="/members" className="text-2xs font-semibold text-signal-blue hover:text-[#005be0]">
                      Xem danh sách
                    </Link>
                  </td>
                </tr>
                ...
                <tr className="hover:bg-cloud/60 transition-colors">
                  ...
                  <td className="py-3 px-3">
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-2xs font-semibold bg-[#e6f0ff] text-signal-blue border border-[#d4e4fa]">
                      Tích cực
                    </span>
                  </td>
                  <td className="py-3 px-4 text-right">
                ...
                <tr className="hover:bg-cloud/60 transition-colors">
                  ...
                  <td className="py-3 px-4 text-right">
                    <Link to="/tasks" className="text-2xs font-semibold text-signal-blue hover:text-[#005be0]">
                      Xem bảng việc
                    </Link>
                  </td>
                </tr>
