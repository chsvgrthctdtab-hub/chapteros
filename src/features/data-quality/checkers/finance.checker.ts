  ...
  async check(organizationId: string): Promise<DataQualityIssue[]> {
    ...
    if (netBalance < 0) {
      issues.push({
        ...
        title: 'Quỹ Chi hội đang bị âm số dư',
        description: `Tổng số dư khả dụng hiện tại của Chi hội đang âm: ${formatVND(netBalance)} (Tổng thu: ${formatVND(totalIncome)}, Tổng chi: ${formatVND(totalExpense)}).`,
        entityType: 'finance',
        entityId: null,
        entityName: 'Sổ quỹ Chi hội',
        detectedAt: nowIso,
