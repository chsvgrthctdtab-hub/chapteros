...
export function TaskCard({
  ...
  return (
    <div
      ...
      className={cn(
        'group bg-white rounded-2xl border transition-all duration-150 shadow-xs hover:shadow-md flex flex-col justify-between overflow-hidden cursor-pointer',
        dueInfo.isOverdue
          ? 'border-rose-300 ring-1 ring-rose-200/50'
          : 'border-hairline hover:border-slate-gray/40'
      )}
    >
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1">
            <span className="font-mono tabular-nums text-[10px] text-mist-gray font-medium mr-1">
              #{taskCode}
            </span>
              ...
              >
                ...
                {onDelete && (
                  <button
                    ...
                    className="p-1 text-slate-gray hover:text-rose-600 hover:bg-rose-50 rounded-md transition-colors cursor-pointer"
                    title="Delete task"
                  >
        ...
        {/* Title & Description */}
        <div>
          <h3 className="text-sm font-semibold text-ink-navy group-hover:text-signal-blue transition-colors line-clamp-2 leading-snug">
            {task.title}
          </h3>
          {task.description && (
            <p className="text-xs text-slate-gray line-clamp-2 mt-1 leading-relaxed">
              {task.description}
            </p>
          )}
        </div>
        ...
        <div className="text-xs">
          {task.activity ? (
            <Link
              ...
              onClick={(e) => e.stopPropagation()}
              className="inline-flex items-center gap-1 text-ink-navy bg-cloud hover:bg-pebble border border-hairline px-2.5 py-0.5 rounded-full font-medium text-[11px] transition-colors max-w-full truncate"
              title={task.activity.title}
            >
              <span className="truncate">{task.activity.title}</span>
            </Link>
          ) : (
            <span className="inline-flex items-center text-mist-gray text-[11px]">
              Nhiệm vụ độc lập
            </span>
          )}
        </div>
      ...
      <div className="bg-cloud/70 px-4 py-2.5 border-t border-hairline flex items-center justify-between gap-2 text-xs">
        {/* Assignee */}
          ...
          {task.assignee ? (
            <>
              {task.assignee.avatarUrl ? (
                ...
                <div className="w-5 h-5 rounded-full bg-[#e6f0ff] text-signal-blue flex items-center justify-center font-bold text-[9px] shrink-0 border border-[#d4e4fa]">
                  {task.assignee.fullName?.charAt(0) || 'U'}
                </div>
              )}
              <div className="min-w-0">
                <p className="font-medium text-ink-navy truncate text-[11px]">
                  {task.assignee.fullName}
                </p>
              </div>
            </>
          ) : (
            <div className="flex items-center gap-1 text-mist-gray text-[11px] italic">
        ...
        <div className="shrink-0 text-right">
          <div
            className={cn(
              ...
              dueInfo.isOverdue
                ? 'text-rose-700 font-semibold bg-rose-50 px-2 py-0.5 rounded-full border border-rose-200'
                : 'text-slate-gray'
            )}
          >
            {dueInfo.isOverdue ? (
