...
export function TermToolbar({
  ...
  return (
    <div
      id="term-toolbar"
      className="flex flex-col md:flex-row md:items-center justify-between gap-3 p-3 rounded-xl border border-hairline bg-white shadow-xs"
    >
      {/* Search Input */}
      <div className="relative flex-1 min-w-[240px]">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-mist-gray pointer-events-none" />
        <Input
          value={searchQuery}
          onChange={(e) => onSearchChange(e.target.value)}
          placeholder="Tìm kiếm theo tên, năm học, mốc thời gian..."
          className="pl-9 pr-8 h-9 text-xs sm:text-sm bg-cloud border-hairline focus:bg-white transition-colors"
        />
        {searchQuery && (
          <button
            onClick={() => onSearchChange('')}
            className="absolute right-2.5 top-1/2 -translate-y-1/2 text-mist-gray hover:text-slate-gray p-0.5 cursor-pointer"
          >
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          <SelectTrigger className="h-8.5 text-xs bg-cloud border-hairline w-auto min-w-[140px] text-ink-navy">
        ...
        <div className="h-5 w-px bg-hairline mx-0.5 hidden sm:block" />

        {/* View Mode Toggle Buttons */}
        <div className="flex items-center bg-pebble p-0.5 rounded-lg border border-hairline">
          <button
            type="button"
