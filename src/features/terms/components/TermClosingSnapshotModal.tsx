  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col p-0 gap-0 overflow-hidden">
                  ...
                  <DialogDescription className="text-xs text-mist-gray mt-1 flex flex-wrap items-center gap-x-4 gap-y-1">
                    ...
                    {(snapshot.closedByName || snapshot.closedBy) && (
                      <span className="flex items-center gap-1">
                        ...
                        Người thực hiện đóng: <strong className="text-slate-gray">{snapshot.closedByName || snapshot.closedBy}</strong>
                      </span>
                    )}
                  </DialogDescription>
                </div>
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-cloud">
          ...
          {snapshot.handoverNotes && (
            <div className="p-4 bg-white border border-hairline rounded-xl space-y-1.5 shadow-xs">
              <div className="text-xs font-bold text-ink-navy flex items-center gap-1.5">
                <FileSpreadsheet className="h-4 w-4 text-signal-blue" />
                Biên bản / Ghi chú bàn giao nhiệm kỳ
              </div>
              <p className="text-xs text-slate-gray whitespace-pre-wrap leading-relaxed">
                {snapshot.handoverNotes}
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-white p-3.5 rounded-xl border border-hairline shadow-xs">
              <div className="text-[11px] font-medium text-mist-gray flex items-center gap-1">
                <Users className="h-3.5 w-3.5 text-signal-blue" />
                Tổng Hội viên
              </div>
              <div className="text-xl font-bold text-ink-navy mt-1">
                {stats.members.total}
              </div>
              <div className="text-[10px] text-mist-gray mt-0.5">
          ...
          {keyOfficers.length > 0 && (
            <div className="bg-white rounded-xl border border-hairline p-4 space-y-3 shadow-xs">
              <div className="text-xs font-bold text-ink-navy flex items-center gap-1.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
                {keyOfficers.map((member, idx) => (
                  <div
                    key={idx}
                    className="p-2.5 bg-cloud rounded-lg border border-hairline text-xs"
                  >
                    <div className="font-semibold text-ink-navy">{member.fullName}</div>
                    <div className="text-signal-blue font-medium text-[11px]">{member.position}</div>
                    {member.studentId && (
                      <div className="text-mist-gray text-[10px]">MSSV: {member.studentId}</div>
                    )}
                  </div>
                ))}
              </div>
          ...
          {snapshot.activitiesList && snapshot.activitiesList.length > 0 && (
            <div className="bg-white rounded-xl border border-hairline p-4 space-y-3 shadow-xs">
              ...
              <div className="max-h-60 overflow-y-auto divide-y divide-hairline text-xs">
                {snapshot.activitiesList.map((act) => (
                  <div key={act.id} className="py-2 flex items-center justify-between gap-2">
                    <div>
                      <div className="font-semibold text-ink-navy">{act.title}</div>
